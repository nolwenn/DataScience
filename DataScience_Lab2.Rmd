...
```{r decompose, solution = TRUE, eval=FALSE}
...
*Utilisez `ggplot`pour visualisez ces composantes sur un même graphique*

```{r ggplotmydec, eval=FALSE}
...
```{r, solution = TRUE}
# les variations hebdomadaires sont négligeables par rapport à la tendance. Les variations "aléatoires" ne semblent pas l'être totalement et semblent être plus importante lors des phases d'incidence forte -> processus multiplicatifs plutôt qu'additif
```
...
```{r aggr, solution = TRUE, eval=FALSE}
...
```{r plotagg, solution  = TRUE, eval=FALSE}
...
```{r rollmean, solution = TRUE, eval=FALSE}
library(zoo)
...
```{r sumByMonthParis, eval=TRUE, solution=TRUE}
...
Dans un premier temps, nous allons aggréger les incidences d'hospitalisation  avec l'instruction suivante. Quel(s) est(sont) le niveau d'aggrégation?
...
- Suivez les instructions ci-dessous pourse faire.

**Présentez la carte en retirant les départements ultra-marins.**

```{r sans ultra-marin, eval=FALSE}
...
```{r carte sans ultra-marin, solution = TRUE, eval=FALSE}
